...
public class RobotHardwareInitializer {
    ...
    public enum MotorComponent implements Component<DcMotor> {
        ...
        @Deprecated INTAKE_MOTOR("intake"),
        @Deprecated UPPIES("uppies"),
        @Deprecated EXTENSION_VIPER("viper_extension"),
        ;
    ...
    /*
    public enum CameraComponent implements Component<Camera> {
        CAMERA_1("Camera 1");

        private final String componentName;

        CameraComponent(String componentName) { this.componentName = componentName; }

        @Override public String getComponentName() { return componentName; }
        @Override public CameraName get(HardwareMap map) { return map.get(CameraName.class, componentName); }
    }
     */
